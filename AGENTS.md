...
cd apps/frontend-web && npx tsc --noEmit    # type check
```
...
- Güvenlik taraması için ayrı hook **yok** — secret koruması `.gitignore` (fcm/env/pem/key) +
  `.Codex/settings.json` deny listesi ile sağlanıyor. Yeni secret'ı asla koda gömme,

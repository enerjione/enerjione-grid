    ...
    return AlarmRuleRead.model_validate(
        {
            ...
            "notify_whatsapp_web": row.notify_whatsapp_web,
...
def create_alarm_rule(
    ...
    record_event(
        ...
        message=f"Alarm rule created: {row.name} ({row.signal_key})",
        metadata={
            "rule_id": row.id,
            "signal_key": row.signal_key,
            "level": row.level,
...
def update_alarm_rule(
    ...
    record_event(
        ...
        severity="info",
        actor_username=current_user.username,
        message=f"Alarm rule updated: {row.name}",
...
def delete_alarm_rule(
    ...
    record_event(
        ...
        severity="warning",
        actor_username=current_user.username,
        message=f"Alarm rule deleted: {name} ({sig})",
